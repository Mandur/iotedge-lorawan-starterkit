...
namespace LoRaWan
{
    ...
    public readonly record struct DevAddr
    {
        ...
        private static uint SetNetworkId(uint devAddr, int value, [CallerArgumentExpression(nameof(value))] string? paramName = null) =>
            value is >= 0 and <= MaxNetworkId
            ? (devAddr & NetworkAddressMask) | unchecked((uint)value << 25)
            : throw new ArgumentException(null, paramName);

        private static uint SetNetworkAddress(uint devAddr, int value, [CallerArgumentExpression(nameof(value))] string? paramName = null) =>
